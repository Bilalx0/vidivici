        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none" />
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-1">Rating</label>
              <select value={rating} onChange={(e) => setRating(parseInt(e.target.value))} className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none">
          ...
          <div>
            ...
            <textarea rows={3} value={content} onChange={(e) => setContent(e.target.value)} className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none resize-none" />
          </div>
          ...
          {testimonials.map((t) => (
            <div key={t.id} className="bg-white border border-gray-200 rounded-xl p-6">
              <div className="flex justify-between items-start mb-3">
                ...
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${t.isVisible ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-500"}`}>
                  {t.isVisible ? "Visible" : "Hidden"}
                </span>
              </div>
              <p className="text-sm text-gray-500 italic mb-4">&ldquo;{t.content}&rdquo;</p>
              <div className="flex gap-4">
                <button onClick={() => handleEdit(t)} className="text-xs text-gray-900 font-medium hover:underline">Edit</button>
                <button onClick={() => toggleVisibility(t)} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">{t.isVisible ? "Hide" : "Show"}</button>
