...
export default function AdminDashboard() {
  return (
    <div>
      <h1 className="text-2xl font-bold text-mist-900 mb-8">Dashboard</h1>
        ...
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="bg-white border border-gray-200 rounded-xl p-6">
              ...
              <p className="text-2xl font-bold text-mist-900 mb-1">{stat.value}</p>
              <p className="text-sm text-mist-500">{stat.label}</p>
            </div>
          )
        })}
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {recentBookings.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-mist-700">{b.customer}</td>
                  <td className="px-6 py-4 text-sm text-mist-500">{b.car}</td>
                  <td className="px-6 py-4 text-sm text-mist-500">{b.date}</td>
