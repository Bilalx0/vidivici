  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <span className="text-sm text-mist-500">{messages.filter(m => !m.read).length} unread</span>
      </div>
            ...
            <button onClick={() => setExpanded(expanded === msg.id ? null : msg.id)} className="w-full p-5 text-left flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center mb-1">
                  <p className="text-sm font-medium text-mist-900">{msg.name} <span className="text-mist-400 font-normal">-- {msg.subject}</span></p>
                  <span className="text-xs text-mist-400 ml-4 flex-shrink-0">{msg.date}</span>
                </div>
            ...
            {expanded === msg.id && (
              <div className="px-5 pb-5 border-t border-gray-200 pt-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="text-xs text-mist-500 hover:text-mist-900 transition-colors">{msg.read ? "Mark Unread" : "Mark Read"}</button>
