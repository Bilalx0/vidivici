  ...
  return (
    <div className="relative flex flex-col bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 group">
        ...
        <button
          ...
          className={`absolute top-3 right-3 w-8 h-8 rounded-full backdrop-blur-sm flex items-center justify-center transition-all duration-200 ${
            wishlisted
              ...
              : "bg-white/80 text-mist-400 hover:text-red-500"
          }`}
        >
      ...
      <div className="flex flex-col gap-2 px-4 pt-3.5 pb-4">
        <p className="text-[10px] text-mist-400 font-medium tracking-wide uppercase">{brand}</p>
        <h3 className="text-[15px] font-semibold text-mist-900 leading-snug -mt-0.5">{name}</h3>

        {/* Stats */}
        <div className="flex items-center justify-between mt-1">
          <div className="flex flex-col items-center gap-0.5">
            <Users size={12} className="text-mist-400" />
          ...
          <div className="flex flex-col items-center gap-0.5">
            ...
            <span className="text-[11px] font-semibold text-mist-700">{horsepower ? `${horsepower} hp` : "—"}</span>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between mt-1">
          <Link href={`/cars/${slug}`} className="flex items-center gap-1 text-[11px] font-semibold text-mist-500 hover:text-mist-900 transition-colors">
            View Details <ArrowUpRight size={11} strokeWidth={2.5} />
          </Link>
          <div className="flex flex-col items-end">
            <span className="text-[15px] font-bold text-mist-900">${pricePerDay}</span>
            <span className="text-[10px] text-mist-400">/day</span>
          </div>
        </div>
      </div>
    </div>
