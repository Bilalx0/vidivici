    ...
    <div className="relative flex flex-col bg-white rounded-3xl 2xl:rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-xs 2xl:w-[450px] flex-shrink-0 group cursor-pointer">
      <div className="relative h-56 2xl:h-[300px] overflow-hidden p-3 2xl:p-4">
        <Link href={`/cars/${car.slug}`} className="block w-full h-full">
          {img ? (
            <img src={img} alt={car.name} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 rounded-2xl 2xl:rounded-2xl" />
          ) : (
            <div className="w-full h-full rounded-2xl 2xl:rounded-2xl flex items-center justify-center bg-mist-100">
        ...
        <button
          onClick={() => { }}
          className="absolute top-5 right-5 w-8 h-8 2xl:w-11 2xl:h-11 rounded-full flex items-center justify-center backdrop-blur-sm transition-all duration-200 bg-mist-700 text-red-500"
        >
          <Heart size={13} className="2xl:w-3.5 2xl:h-3.5" fill="currentColor" strokeWidth={2} />
        </button>
      </div>

      <div className="flex flex-col gap-2 2xl:gap-4 px-6 2xl:px-8 pt-3.5 2xl:pt-5 pb-4 2xl:pb-6">
        <p className="text-xs 2xl:text-sm text-mist-400 font-medium tracking-wide uppercase truncate">{car.brand.name}</p>
        <h3 className="text-lg sm:text-xl 2xl:text-2xl font-semibold text-mist-900 leading-snug -mt-0.5 line-clamp-1">{car.name}</h3>
        ...
        <div className="flex items-center justify-between mt-0.5 2xl:mt-0">
          <Link href={`/cars/${car.slug}`} className="flex items-center gap-1 2xl:gap-2.5 text-sm 2xl:text-lg text-mist-500 hover:text-mist-900 transition-colors">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-[10px] 2xl:text-sm text-mist-400 line-through">${Math.round(car.pricePerDay * 1.2).toLocaleString()} / day</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="relative flex flex-col bg-white rounded-3xl 2xl:rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-xs 2xl:w-[450px] flex-shrink-0 group cursor-pointer">
      <div className="relative h-56 2xl:h-[300px] overflow-hidden p-3 2xl:p-4">
        <Link href={`/villas/${villa.slug}`} className="block w-full h-full">
          {img ? (
            <img src={img} alt={villa.name} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 rounded-2xl 2xl:rounded-2xl" />
          ) : (
            <div className="w-full h-full bg-mist-100 flex items-center justify-center text-mist-400 text-sm rounded-2xl 2xl:rounded-2xl">No Image</div>
          )}
        </Link>
        <button
          onClick={() => { }}
      ...
      <div className="flex flex-col gap-2 2xl:gap-4 px-6 2xl:px-8 pt-3.5 2xl:pt-5 pb-4 2xl:pb-6">
        ...
        <h3 className="text-lg sm:text-xl 2xl:text-2xl font-semibold text-mist-900 leading-snug -mt-0.5">{villa.name}</h3>
        ...
        <div className="flex items-center justify-between mt-0.5 2xl:mt-0">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-[10px] 2xl:text-sm text-mist-400">/night</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="relative flex flex-col bg-white rounded-3xl 2xl:rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-xs 2xl:w-[450px] flex-shrink-0 group cursor-pointer">
      <div className="relative h-56 2xl:h-[300px] overflow-hidden p-3 2xl:p-4">
        <Link href={`/events/${event.slug}`} className="block w-full h-full">
          {img ? (
            <img src={img} alt={event.name} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 rounded-2xl 2xl:rounded-2xl" />
          ) : (
            <div className="w-full h-full bg-mist-100 flex items-center justify-center text-mist-400 text-sm 2xl:text-base rounded-2xl 2xl:rounded-2xl">No Image</div>
          )}
        </Link>
        <button
          onClick={() => { }}
      ...
      <div className="flex flex-col gap-2 2xl:gap-4 px-6 2xl:px-8 pt-3.5 2xl:pt-5 pb-4 2xl:pb-6">
        ...
        <div className="flex items-center justify-between mt-0.5 2xl:mt-0">
          <Link href={`/events/${event.slug}`} className="flex items-center gap-1 2xl:gap-2.5 text-sm 2xl:text-lg text-mist-500 hover:text-mist-900 transition-colors">
            View Details <ArrowUpRight size={11} className="2xl:w-4.5 2xl:h-4.5" strokeWidth={2.5} />
          </Link>
        </div>
      </div>
