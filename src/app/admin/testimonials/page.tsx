        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" className="w-full bg-gray-50 border border-gray-200 text-mist-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none" />
            </div>
            <div>
              <label className="text-xs text-mist-500 block mb-1">Rating</label>
              <select className="w-full bg-gray-50 border border-gray-200 text-mist-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none">
          ...
          <div>
            ...
            <textarea rows={3} className="w-full bg-gray-50 border border-gray-200 text-mist-900 text-sm px-4 py-3 rounded-lg focus:border-black focus:outline-none resize-none" />
          </div>
        ...
        {testimonialsList.map((t) => (
          <div key={t.id} className="bg-white border border-gray-200 rounded-xl p-6">
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${t.visible ? "bg-green-50 text-green-600" : "bg-gray-100 text-mist-500"}`}>
                {t.visible ? "Visible" : "Hidden"}
              </span>
            </div>
            <p className="text-sm text-mist-500 italic mb-4">&ldquo;{t.content}&rdquo;</p>
            <div className="flex gap-4">
              <button className="text-xs text-mist-900 font-medium hover:underline">Edit</button>
              <button className="text-xs text-mist-400 hover:text-mist-600 transition-colors">{t.visible ? "Hide" : "Show"}</button>
